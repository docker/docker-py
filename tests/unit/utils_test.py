...
class ConverVolumeBindsTest(unittest.TestCase):
    ...
    def test_convert_volume_binds_unicode_bytes_input(self):
        ...
        data = {"/mnt/지연".encode(): {"bind": "/unicode/박".encode(), "mode": "rw"}}
...
class ParseHostTest(unittest.TestCase):
    ...
    def test_parse_host_empty_value(self):
        ...
        for val in [None, ""]:
            assert parse_host(val, is_win32=False) == DEFAULT_UNIX_SOCKET
            assert parse_host(val, is_win32=True) == npipe

    def test_parse_host_unix_empty_scheme(self):
        """Test that 'unix://' falls back to DEFAULT_UNIX_SOCKET."""
        from docker.constants import DEFAULT_UNIX_SOCKET

        assert parse_host("unix://") == DEFAULT_UNIX_SOCKET
...
class PortsTest(unittest.TestCase):
    ...
    def test_split_port_with_protocol(self):
        for protocol in ["tcp", "udp", "sctp"]:
            internal_port, external_port = split_port(f"127.0.0.1:1000:2000/{protocol}")

...
class ContainerCollectionTest(BaseIntegrationTest):
    ...
    def test_run_with_network(self):
        ...
        container = client.containers.run(
            'alpine', 'echo hello world', network=net_name, network_alias=['alias1', 'alias2'],
            detach=True
        )
        self.tmp_containers.append(container.id)
        ...
        assert 'alias1' in attrs['NetworkSettings']['Networks'][net_name]['Aliases']
        assert 'alias2' in attrs['NetworkSettings']['Networks'][net_name]['Aliases']

...
def mkbuildcontext(dockerfile, file_list=None):
    ...
    if(file_list):
        for fname in file_list:
            if not(os.path.exists(fname)):
                raise IOError(fname + " not found, working dir: "+os.getcwd())
            if(os.path.isdir(fname)):
                file_list.extend([fname+"/"+x for x in listdir(fname)])
            if(os.path.isfile(fname)):
                fli = open(fname, 'r')
                arcname = os.path.basename(fname)
                fli_info = t.gettarinfo(fileobj=fli, arcname=arcname)
                t.addfile(fli_info, fli)
                fli.close()
    t.close()
    f.seek(0)
    return f
